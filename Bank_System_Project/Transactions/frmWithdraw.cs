...
namespace MyBankSystemDesktopApp.Transactions
{
    public partial class frmWithdraw : Form
    {
        ...
        private bool _Add_Transaction_To_DataBase()
        {
            ...
            CurrentTransaction.Transaction_Type_ID = (int)eTransactionType.Withdrawl;
            CurrentTransaction.Sender_Acc = Convert.ToInt32(CurrentClient.AccountNumber);
            CurrentTransaction.Reciver_Acc = null;
        ...
        private void _Withdraw()
        {
            ...
            if(CurrentClient.Balance < Convert.ToDouble(txtAmount.Text))
            {
                MessageBox.Show("Withdraw Failed ,withdrawl Amount should not exceed : $" + CurrentClient.Balance );
                return;
            }
                                ...
                                "Validating", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
            {
                ...
                {
                    if (clsTransaction.Withdrawl(CurrentClient.Client_ID, Convert.ToDouble(txtAmount.Text)))
                    {
